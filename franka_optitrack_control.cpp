...
bool isMovementSafe(Eigen::Vector3d& target_position,
    ...
    double angle_diff = previous_orientation.angularDistance(target_orientation);

    if (position_diff > 0.5 || angle_diff > M_PI/3.0) {  // 50cm, 60度
...
int main(int argc, char **argv) {
    ...
    ros::Subscriber sub_pose = node_handle.subscribe<geometry_msgs::PoseStamped>(
        ...
        [&tf_broadcaster, &marker_pub, &hand_pose, &hand_pose_mutex, &move_group](
            ...
            // 计算相对运动
            Eigen::Affine3d relative_motion = 
                current_optitrack * initial_poses.optitrack_pose.inverse();

            // 将相对运动应用到flange初始位置
            Eigen::Affine3d target_pose = initial_poses.flange_pose * relative_motion;

            // 只发布可视化，不发送控制命令
            publishVisualization(tf_broadcaster, marker_pub, 
                               current_optitrack, target_pose);
    ...
    while (ros::ok()) {
        ...
        {
            // Acquire lock to read the latest hand pose
            std::lock_guard<std::mutex> lock(hand_pose_mutex);
            target_position = hand_pose.translation();
            target_orientation = Eigen::Quaterniond(hand_pose.linear());
        }
    ...
    if (std::abs(target_position.x() - (workspace_center.x() + workspace_size.x() * 0.5)) < boundary_threshold ||
        ...
        ik_options.goals.emplace_back(
            new bio_ik::OrientationGoal("franka_flange", 
                tf2::Quaternion(target_orientation.x(), 
